...
export const useDefaultData = (props: { rules: {}; formItem: any[]; postParams: { [x: string]: any }; rowData: { [x: string]: any } }) => {
    ...
    (item: {
      ...
      for (const key in item) {
        ...
        if (key === 'remoteMethod' && item.remoteMethod && item.type === 'select') {
          item.remoteMethod.then((res: any) => {
            item.options = unref(res)
          })
        }
